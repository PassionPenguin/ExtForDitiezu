...
chrome.webNavigation.onCompleted.addListener(() => {
    tabId.forEach((e, index) => {
        chrome.tabs.executeScript(e, {code: "try{setTimeout(()=>{document.querySelector('#loadingAnimation').style.opacity='0';setTimeout(()=>{try{document.body.removeChild(document.querySelector('#loadingAnimation'));}catch(e){}},500);},500);}catch(e){}"});
        tabId = tabId.slice(0, index) + tabId.slice(index + 1, tabId.length);
    });
}, {
    url: [{
        hostContains: 'ditiezu.'
