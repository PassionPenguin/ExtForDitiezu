...
chrome.runtime.onMessageExternal.addListener((request, sender, sendResponse) => {
    ...
    if (request.xhr === true) {
        const xhr = new XMLHttpRequest(),
            ...
            url = request.xhrUrl,
            content = request.xhrContent;

        xhr.open(method, url, true);
        xhr.onreadystatechange = function () {
            // In local files, status is 0 upon success in Mozilla Firefox
            if (xhr.readyState === XMLHttpRequest.DONE) {
                let status = xhr.status, res = xhr.response, resXML = xhr.responseXML;
                if (status === 0 || (status >= 200 && status < 400)) {
                    // The request has been completed successfully
                    sendResponse({status: status, res: res, response: res, responseXML: resXML});
                } else {
...
chrome.webNavigation.onCompleted.addListener(() => {
    chrome.tabs.executeScript(tabId, {code: "setTimeout(()=>{document.querySelector('#loadingAnimation').style.opacity='0';setTimeout(()=>{try{document.body.removeChild(document.querySelector('#loadingAnimation'));}catch(e){}},500);},500);"});
}, {
    url: [{
        hostContains: 'ditiezu.'
    }],
